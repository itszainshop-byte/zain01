...
// Enforce tenant-scoped uniqueness for top-level and grouped slugs.
navigationCategorySchema.index({ tenantId: 1, name: 1 }, { unique: true, collation: { locale: 'en', strength: 2 } });
navigationCategorySchema.index({ tenantId: 1, slug: 1 }, { unique: true, sparse: true, collation: { locale: 'en', strength: 2 } });
navigationCategorySchema.index({ tenantId: 1, 'slugGroups.slug': 1 }, { unique: true, sparse: true, collation: { locale: 'en', strength: 2 } });
...
navigationCategorySchema.pre('save', async function(next) {
  try {
    ...
    if (this.isModified('name') || !this.slug) {
      ...
      while (await this.constructor.findOne({ ...tenantFilter, slug: candidate, _id: { $ne: this._id } })) {
...
navigationCategorySchema.plugin(tenantScopedModel);
